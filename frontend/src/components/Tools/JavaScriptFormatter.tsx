...
import { formatJavaScript, minifyJavaScript } from '@/services/api'
...
export default function JavaScriptFormatter() {
  ...
  const handleProcess = async () => {
    ...
    try {
      const result = mode === 'format'
        ? await formatJavaScript(input, indentSize)
      ...
      setError(err.response?.data?.error || `Failed to ${mode} JavaScript`)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDownload = () => {
    ...
    const filename = mode === 'format' ? 'formatted.js' : 'minified.js'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-center">
        ...
        >
          {loading ? 'Processing...' : `${mode === 'format' ? 'Format' : 'Minify'} JS (Ctrl+Enter)`}
        </button>
      </div>
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {mode === 'format' ? 'Formatted' : 'Minified'} JavaScript
            </label>
            <button
          ...
          <CodeBlock code={output} language="javascript" title="Result" />
        </div>
      )}
    </div>
  )
